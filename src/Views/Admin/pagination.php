...
<html>
    ...
    <body>
        ...
        $result = mysqli_query($conn, 'select count(id_product) from PRODUCT');
        $row = mysqli_fetch_assoc($result);
        $total_records = $row['id_product'];
        ...
        <div>
            ...
            while ($row = mysqli_fetch_assoc($result)){
                echo '<li>' . $row['id_product'] . '</li>';
                echo '<li>' . $row['name_product'] . '</li>';
            }
            ?>
        </div>
        <div class="pagination">
           <?php 
            ...
            if ($current_page > 1 && $total_page > 1){
                echo '<a href="pagination.php?page='.($current_page-1).'">Prev</a> | ';
            }
